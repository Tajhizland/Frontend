...
export default function Page() {
    ...
    async function submit(e: FormData) {
        setLoading(true)
        let response = await upload({model_id: Number(id), file: e.get("file") as File, model_type: "news"}, setProgress)
        if (response?.success) {
            ...
            <div className={"grid grid-cols-1 md:grid-cols-2  xl:grid-cols-5 gap-5 border rounded  mt-10"}>
                {
                    data?.map((item) => (<>
                        <div className="flex flex-col justify-center items-center gap-y-4 ">
                            ...
                            <span>
                             {process.env.NEXT_PUBLIC_IMAGE_BASE_URL}/news/file/{item.path}
                            </span>
