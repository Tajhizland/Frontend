...
const HorizontalProductCard: FC<ProductCardProps> = ({
        ...
        return (
            <div className="flex gap-1  justify-start  w-full">
                {data?.colors.data.map((color, index) => (
                    <div
                        key={index}
                        className={`relative w-4 h-4 rounded-full overflow-hidden z-10 border cursor-pointer`}
    ...
    return (
        <>
            <div
                className={`nc-ProductCard relative flex flex-row items-center bg-transparent group ${className}`}
            >
                ...
                <div
                    ...
                    <LikeButton liked={data?.favorite} likeHandle={likeHandle}
                                className="absolute top-3 end-3 z-10 hidden  "/>
                    <div className={" "}>
                        {renderStatus()}
                    </div>
                    {/*{  renderGroupButtons()}*/}
